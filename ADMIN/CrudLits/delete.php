<?php
// confirmer
if(isset($_POST["id_lit"]) && !empty($_POST["id_lit"])){
    // Inclure le fichier config
    require_once "database.php";
    $id_lit=$_POST["id_lit"];
    // Prepare la requette
    $sql = "DELETE FROM lit WHERE id_lit =?";
    
    if($stmt = mysqli_prepare($link, $sql)){
        // Bind les variables
        mysqli_stmt_bind_param($stmt, "s", $param_id_lit);
        
        // Set parameters
        $param_id_lit = trim($_POST["id_lit"]);
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            echo '
        <script type="text/javascript">
            alert("Oops! une erreur est survenue, Vous ne pouvez pas supprimer ce Lit "); 
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger">
                            <input type="hidden" name="id_lit" value="<?php echo trim($_GET["id_lit"]); ?>"/>
