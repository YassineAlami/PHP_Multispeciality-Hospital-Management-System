...
if (isset($_POST['email']))
{
	...
	$queryTotal = "SELECT  email_user, type_user, id_q FROM `user_app` WHERE email_user='$email' and psw_user='$password'";
	$resultTotal = mysqli_query($conn,$queryTotal) or die(mysql_error());
	...
	$rows = mysqli_num_rows($resultTotal);
	if($rows==1 and $id_q!=null)
	{
		if($type_user == 'Medecin')
		{
			$_SESSION['email'] = $email;
			header("Location: MED/EspaceMedecin.php");
		}
		else if($type_user == 'Patient')
		{
			$_SESSION['email'] = $email;
			header("Location:MED/EspacePatient2.php");
		}
	...
	else if($rows ==1 and $id_q==null)
	{
		// and 'integer' == gettype($id_q))
		$_SESSION['email'] = $email;
		header("Location: MED/changer_mdp.php");
	}
	else
	{
			...
			<div class="wrap-login100">
				...
				<form class="login100-form validate-form" action="" method="post" name="login">
					...
					<?php// if (! empty($message)) { ?>
							<p style="font-size: 30px; color:red;" class="errorMessage"><?php// echo $message; ?></p>
					<?php// } ?>
					-->
					
					<?php if (! empty($message)) { ?>
						<?php
							echo "<span style='color:#ff0000;text-align:center;'>$message!</span> <br>";
					...
					<div class="text-center p-t-12">
						<span class="txt1">
							<a href="MED\changer_mdp2"> Changer le Mot de passe ?</a>
						</span>
					</div>                                                            
	
					<div class="text-center p-t-136">
						<a class="txt2" href="mdp_oub.php">
