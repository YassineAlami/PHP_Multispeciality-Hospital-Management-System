...
require('config.php');
	...
	if (isset($_REQUEST['email'], $_REQUEST['question'], $_REQUEST['repense']))
	{
		...
		$queryQuest = "SELECT rep_q, id_q FROM `user_app` where email_user= '$email'";
		$resultQuest = mysqli_query($conn,$queryQuest) or die(mysql_error());
		
		foreach($resultQuest as $row)
		{
			$rep_q =  $row['rep_q'];
			$id_q = $row['id_q'];
		}
		
		if($repense==$rep_q and $question==$id_q)
		{
			$_SESSION['email'] = $email;
			echo "<script type='text/javascript'>
				window.alert('Réinitialisation de Mot de Passe !');
				window.location.href='changer_mdp3.php';
			</script>";
		}
		else
		{
		...
		<div class="card-body">
		 <div class="card-content p-2">
		  ...
		  <div class="card-title text-uppercase text-center py-3">Réinitialisation Mot de Passe</div>
		    <form action="" method="post">
			  ...
			  <label for="exampleInputEmailId" class="sr-only">Email ID</label>
			   <div class="position-relative has-icon-right">
				  <input type="text" id="exampleInputEmailId" name="email" class="form-control input-shadow" placeholder="Email">
			...
			<div class="form-group" id="divgenre_pt">	
				<div class="position-relative has-icon-right">
					<select name="question" id="genre_pt" class="form-control">
						<option value="0">Question de Sécurité</option>
			...
			<div class="form-group">
				<label for="exampleInputPassword" class="sr-only">Repense</label>
					<div class="position-relative has-icon-right">
						<input type="text" id="exampleInputPassword" name="repense" class="form-control input-shadow" placeholder="Ma Réponse">
						<div class="form-control-position">
							<i class="icon-lock"></i>
						</div>
					</div>
