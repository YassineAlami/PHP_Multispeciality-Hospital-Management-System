...
require('config.php');
	...
	$queryDelCre = "DELETE FROM `creneau` WHERE date_cre < date(now())";
	...
	$queryPts = "SELECT DISTINCT medecin.cin_med, nom_med, pnom_med, lib_spe FROM `medecin` INNER JOIN patient_medecin INNER JOIN specialite on medecin.cin_med = patient_medecin.cin_med and medecin.id_spe = specialite.id_spe WHERE cin_pt = '$cin_pt'";
	...
	$queryDateCre = "SELECT distinct date_cre, dayname(date_cre) as jour from creneau where id_rdv is null";
	$resultDateCre = mysqli_query($conn, $queryDateCre);
	...
	if (isset($_REQUEST['cin_med']))
	{
		...
		$date_cre = stripslashes($_REQUEST['date_cre']);
		$date_cre = mysqli_real_escape_string($conn, $date_cre);
		
		$_SESSION['date_cre'] = $date_cre;
		...
		$_SESSION['pnom_med'] = $pnom_med;
		header("Location: demande_rdv1.php");
	}
  ...
  <div class="content-wrapper">
	...
	<li class="nav-item">
      <form class="search-bar" action="" method="post">
		<select name="cin_med" id="cin_med" " class="form-control" onchange="showDiv('date_cre', this)">
		...
		<select name="date_cre" id="date_cre" " class="form-control" style="display:none;" onchange="showDiv('submit', this)">
            <option value="0">Date</option>
            <?php while($row1 = mysqli_fetch_array($resultDateCre)):;?>
		...
		<div class="form-actions form-group"><input type="submit" id="submit" name="submit" class="btn btn-success btn-sm" style="display:none;"  value="Suivant"></div> 
      </form>
    </li>
