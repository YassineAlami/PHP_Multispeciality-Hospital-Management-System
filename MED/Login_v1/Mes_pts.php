...
session_start();
    ...
    $queryMedPt = "SELECT patient.cin_pt,nom_pt, pnom_pt, genre_pt, date_n_pt, tel_pt, email_pt FROM patient INNER JOIN patient_medecin ON patient.cin_pt=patient_medecin.cin_pt WHERE patient_medecin.cin_med='$cin_med'";
	$resultMed = mysqli_query($conn,$queryMedPt) or die(mysql_error());
	
	
if (isset($_REQUEST['search'])and 1==2)
{
	// récupérer le nom d'utilisateur et supprimer les antislashes ajoutés par le formulaire
	$cin_pt = stripslashes($_REQUEST['search']);
	$cin_pt = mysqli_real_escape_string($conn, $cin_pt);
	
	//requéte SQL + mot de passe (crypté)
	$queryMedPt = "SELECT patient.cin_pt,nom_pt, pnom_pt, genre_pt, date_n_pt, tel_pt, email_pt FROM patient INNER JOIN patient_medecin ON patient.cin_pt=patient_medecin.cin_pt WHERE patient_medecin.cin_med='$cin_med' and patient_medecin.cin_pt='$cin_pt'";
                    ...
                    <div class="header-left">
                        ...
                        <div class="form-inline">
                            <form class="search-form" method="post">
                                ...
                                <button class="search-close" type="submit">Chercher  <i class="fa fa-close"></i></button>
...
*/
		...
		*/
                    ...
                    if($resultN = mysqli_query($conn, $queryMedPt))
					{
                        if(mysqli_num_rows($resultN) > 0){
                            echo '<table class="table table-bordered table-striped">';
                                ...
                                while($row = mysqli_fetch_array($resultN)){
                                    echo "<tr>";
                                        echo "<td>" . $row['cin_pt'] . "</td>";
                                        echo "<td>" . $row['nom_pt'] . "</td>";
                                        echo "<td>" . $row['pnom_pt'] . "</td>";
                                        echo "<td>" . $row['genre_pt'] . "</td>";
                                        echo "<td>" . $row['date_n_pt'] . "</td>";
                                        echo "<td>" . $row['tel_pt'] . "</td>";
										echo "<td>" . $row['email_pt'] . "</td>";
                                     
                                        echo "<td>";
                                            echo '<a href="CrudPatient/read.php?cin_pt='. $row['cin_pt'] .'" class="me-3" ><span style="color:blue" class="bi bi-eye"></span></a>';
                                            echo '<a href="CrudPatient/update.php?cin_pt='. $row['cin_pt'] .'" class="me-3" ><span style="color:black" class="bi bi-pencil"></span></a>';
                                            echo '<a href="CrudPatient/delete.php?cin_pt='. $row['cin_pt'] .'" ><span style="color:red"class="bi bi-trash"></span></a>';
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";                            
