...
if (isset($_REQUEST['date_temps'], $_REQUEST['heure_d_temps'], $_REQUEST['heure_f_temps']))
{

	// récupérer le nom d'utilisateur et supprimer les antislashes ajoutés par le formulaire
	$date_temps = stripslashes($_REQUEST['date_temps']);
	$date_temps = mysqli_real_escape_string($conn, $date_temps);
	// récupérer l'email et supprimer les antislashes ajoutés par le formulaire
	$heure_d_temps = stripslashes($_REQUEST['heure_d_temps']);
	$heure_d_temps = mysqli_real_escape_string($conn, $heure_d_temps);
	// récupérer le mot de passe et supprimer les antislashes ajoutés par le formulaire
	$heure_f_temps = stripslashes($_REQUEST['heure_f_temps']);
	$heure_f_temps = mysqli_real_escape_string($conn, $heure_f_temps);
	
	$query = "INSERT into `temps_personnel` (cin_med, date_temps, heure_d_temps, heure_f_temps)
		VALUES ('$cin_med', '$date_temps', '$heure_d_temps', '$heure_f_temps')";
	...
	if($res)
	{
		echo "<script type='text/javascript'>
			window.alert('Le Temps Personnel a été Enregistré avec Succès!!');
			window.location.href='ajouter_temps.php';
		</script>";
	}
}else{
?>
    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="breadcrumbs" style="margin-bottom: 25px;">
		 ...
		 <div class="card">
                                                        <div class="card-header">Temps Personnel</div>
															...
															<form action="" method="post">
																...
																<div class="form-group" id="divDateE">
                                                                    <div class="input-group">
                                                                        ...
                                                                        <input type="date" id="date_temps" name="date_temps" placeholder="La date d'entree (YYYY-MM-DD)" class="form-control">
                                                                    </div>
                                                                </div>
																...
																<div class="form-group" id="divDateE">
                                                                    <div class="input-group">
                                                                        <div class="input-group-addon"><i class="fa fa-user"></i></div>
                                                                        <input type="time" id="heure_d_temps" name="heure_d_temps" placeholder="La date d'entree (YYYY-MM-DD)" class="form-control">
                                                                    </div>
                                                                </div>
																...
																<div class="form-group" id="divDateE">
                                                                    <div class="input-group">
                                                                        <div class="input-group-addon"><i class="fa fa-user"></i></div>
                                                                        <input type="time" id="heure_f_temps" name="heure_f_temps" placeholder="La date d'entree (YYYY-MM-DD)" class="form-control">
                                                                    </div>
                                                                </div>
																

                                                                <div class="input-group-addon"> <input type="submit" name="submit" onclick="ok($ok)" class="btn btn-success btn-sm" value="Ajouter"></div>
                     </form>
