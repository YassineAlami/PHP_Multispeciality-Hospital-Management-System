<?php
	...
	else
	{
		...
		$query = "SELECT count(id_rdv) as nbr_rdv_y FROM rendez_vous WHERE cin_pt='$cin_pt' and confirme='Y' or confirme='Z'";
		$result = mysqli_query($conn,$query) or die(mysql_error());
		foreach($result as $row)
		{
		...
		$query = "SELECT count(id_rdv) as nbr_rdv_n FROM rendez_vous WHERE cin_pt='$cin_pt' and confirme='N' or  confirme='X'";
		$result = mysqli_query($conn,$query) or die(mysql_error());
		foreach($result as $row)
		{
 ...
 <div id="wrapper">
  ...
  <!--Start sidebar-wrapper-->
   <div id="sidebar-wrapper" data-simplebar="" data-simplebar-auto-hide="true">
     <div class="brand-logo">
      <a href="EspacePatient2.php">
       ...
       <h5 class="logo-text">Hospital H</h5>
     </a>
   </div>
   <ul class="sidebar-menu do-nicescrol">
      ...
      <li>
        <a href="mes_rdv.php">
        <i class="zmdi zmdi-grid"></i> <span>Mes RDVs</span>
		<?php
		
			$query = "SELECT count(id_rdv) as nbrRdv FROM `Rendez_Vous` WHERE confirme='Z'";
			$result = mysqli_query($conn,$query) or die(mysql_error());
			foreach($result as $row)
			{
				$newRdv =  $row['nbrRdv'];
			...
			if($newRdv>0)
			{
				echo 	"<script type='text/javascript'>
										alert('Nouveau Rendez-Vous Confirmé!');
										</script>";
       ...
       <li class="dropdown-item user-details">
        <a href="javaScript:void();">
           <div class="media">
            ...
            <h6 class="mt-2 user-title"><?php echo $nom_pt." ".$pnom_pt ?></h6>
        ...
        <li class="dropdown-divider"></li>
        <a class="nav-link" href="logout.php"><li class="dropdown-item"><i class="icon-power mr-2" ></i> Logout</li></a>
      </ul>
    </li>
  </ul>
</nav>
                    ...
                    <div class="row">
						...
						<div class="col-md-12">
                            <h5 class="mt-2 mb-3"><span class="fa fa-clock-o ion-clock float-right"></span> Prochains Rendez-Vous</h5>
                             <div class="table-responsive">
                            <?php 
								$queryMedServ = "SELECT nom_med, pnom_med, lbl_type_trai, date_rdv, heure_rdv, heure_f_rdv FROM medecin INNER JOIN rendez_vous ON medecin.cin_med=rendez_vous.cin_med WHERE date_rdv > date(now()) and cin_pt='$cin_pt' and confirme='Y' or confirme= 'Z' ORDER BY date_rdv DESC LIMIT 5";
								$resultMedServ = mysqli_query($conn,$queryMedServ) or die(mysql_error());
					...
					<div class="tab-pane" id="messages">
					   ...
					   <button type="button" class="close" data-dismiss="alert">&times;</button>
						...
						<div class="alert-message">
						  <span><strong>Info!</strong> Ici vous Trouverez les Actualités en Relation avec vos RDVs</span>
						</div>
					  </div>
					  <div class="table-responsive">
						<table class="table table-hover table-striped">
							<tbody>                                    
								...
								<?php 
								$queryMedServ = "SELECT nom_med, pnom_med, lbl_type_trai, date_rdv, heure_rdv, heure_f_rdv FROM medecin INNER JOIN rendez_vous ON medecin.cin_med=rendez_vous.cin_med WHERE confirme='Z' and cin_pt='$cin_pt' ORDER BY date_rdv DESC LIMIT 5";
								$resultMedServ = mysqli_query($conn,$queryMedServ) or die(mysql_error());
								...
								if($resultN = mysqli_query($conn, $queryMedServ))
								{
								    ...
								    while($row = mysqli_fetch_array($resultN))
									{
										echo "<tr>";
										echo "<td>";
										echo "Le Rdv avec Dr. <strong>".$row['nom_med']." ".$row['pnom_med']."</strong> ( ".$row['date_rdv']." de ".$row['heure_rdv']." a ".$row['heure_f_rdv']." est confirmé )";
										echo "</td>";
										echo "</tr>";
									}
								}
...
<?php
	...
	$query = "update `rendez_vous` set confirme='Y' where confirme='Z'";
